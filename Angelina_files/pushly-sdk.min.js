...
PushlySDK.watch('ready', () => {
    try {
        navigator.serviceWorker.getRegistrations().then(function(registrations) {
            for (let registration of registrations) {
                if (registration.scope.match('.*indigitall.*')) {
    ...
    try {
        if (document.location.pathname !== '/') {
            const qs = [...document.querySelectorAll('meta[name="keywords"], meta[name="news_keywords"]')];

            if (qs && qs.length > 0) {
                PushlySDK.push(['page_tag_visit', [...new Set(qs.flatMap(n => n.content.split(',').map(v => v.trim().toLowerCase()).filter(v => v)))]]);
            }
        }
    } catch (e) {}
});
